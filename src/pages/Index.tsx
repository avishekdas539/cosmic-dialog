...
Here’s what I found and how to proceed:\n\n${toolSummary || "- No external calls required"}\n\n> Tip: You can use markdown — like **bold**, _italics_, lists, and code.\n\nExample code:\n\n\`\`\`ts\nfunction hello(name: string) {\n  return \`Hello, ${name}!\`;\n}\n\nconsole.log(hello("world"))\n\`\`\``;
    ...
    const aiMsg: Message = { id: crypto.randomUUID(), role: "assistant", content: answer, ts: now() };
    ...
    <div className="min-h-screen bg-app-gradient">
      ...
      >
        <div className="mx-auto max-w-5xl px-4 py-4 flex items-center justify-between">
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight">Cosmic Dialog</h1>
            ...
            <CardContent className="pt-4">
              <div ref={scrollRef as any} className="h-[52vh] sm:h-[60vh] overflow-hidden">
                <ScrollArea className="h-full pr-3">
                  <div className="space-y-3">
                    {messages.map((m) => (
                      <ChatMessage key={m.id} role={m.role} content={m.content} timestamp={m.ts} />
                    ))}
                  </div>
                </ScrollArea>
              </div>
              ...
              <div className="mt-4 rounded-xl border bg-card/70 p-3 backdrop-blur">
                ...
                <div className="mt-3 flex items-center justify-end gap-2">
                  ...
                  <Button onClick={onSend} className="hover-scale">Send</Button>
                </div>
              </div>
            </CardContent>
          </Card>
